    ...
    <div class="mb-3 flex flex-wrap gap-2">
        @foreach ($images as $image)
            <figure class="relative w-24 overflow-hidden rounded-lg border border-gray-200 bg-white" x-show="!removedImages.includes({{ $image->id }})" data-saved-image="{{ $image->id }}">
                <img src="{{ \Illuminate\Support\Facades\Storage::disk(config('filesystems.image_disk'))->url($image->path) }}" alt="{{ $image->alt_text }}" class="aspect-square w-full object-cover">

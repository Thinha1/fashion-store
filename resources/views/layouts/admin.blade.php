...
<body class="min-h-screen bg-gray-50 text-gray-900 antialiased" x-data="{ sidebarOpen: false, desktop: window.innerWidth >= 1024 }" x-effect="document.body.style.overflow = sidebarOpen && !desktop ? 'hidden' : ''" x-on:resize.window="desktop = window.innerWidth >= 1024; if (desktop) sidebarOpen = false" x-on:keydown.escape.window="if (sidebarOpen) { sidebarOpen = false; $nextTick(() => $refs.menuButton.focus()); }">
    ...
    <aside id="admin-navigation" class="admin-sidebar" data-open="false" :data-open="sidebarOpen ? 'true' : 'false'" x-bind:inert="!sidebarOpen && !desktop">
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-6"><x-brand /><button type="button" x-ref="closeMenu" class="rounded-lg p-2 text-gray-500 lg:hidden" aria-label="Đóng menu" x-on:click="sidebarOpen = false; $nextTick(() => $refs.menuButton.focus())"><x-icon name="close" /></button></div>
        <nav aria-label="Điều hướng quản trị" class="flex-1 space-y-7 overflow-y-auto px-4 py-6">
            <div><p class="sidebar-label">Không gian làm việc</p><a href="{{ route('admin.dashboard') }}" class="sidebar-link" @if(request()->routeIs('admin.dashboard')) aria-current="page" @endif><x-icon name="grid" /> Tổng quan</a></div>
        ...
        <div class="m-4 flex items-center gap-3 rounded-xl bg-gray-50 p-3"><span class="flex size-9 shrink-0 items-center justify-center rounded-full bg-brand-soft text-sm font-semibold text-brand">{{ mb_substr(auth()->user()->name, 0, 1) }}</span><div class="min-w-0"><p class="truncate text-sm font-semibold">{{ auth()->user()->name }}</p><p class="text-xs text-gray-500">Không gian quản trị</p></div></div>
    </aside>
    <div class="min-w-0 lg:pl-64" x-bind:inert="sidebarOpen && !desktop">
        <header class="sticky top-0 z-20 flex min-h-20 items-center justify-between gap-3 border-b border-gray-200 bg-white/95 px-4 backdrop-blur-sm sm:px-8">
